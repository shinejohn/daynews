...
type Task = Task['Row']
type TaskInsert = Task['Insert']
type TaskUpdate = Task['Update']
...
export async function getTaskById(id: string) {
  const supabase = await createServerClient()
  return supabase.from('tasks').select('*').eq('id' as any, id as any).single()
}

export async function getTasksByUser(userId: string) {
  const supabase = await createServerClient()
  return supabase.from('tasks').select('*').eq('user_id' as any, userId as any)
}
