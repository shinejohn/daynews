...
export const CommunitySwitcher = ({
  ...
  return <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 shadow-sm">
          ...
          {searchResults.length > 0 && <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-md shadow-lg border border-gray-200 z-10">
              <ul className="py-1">
                {searchResults.map(community => <li key={community.id}>
                    <button type="button" onClick={() => handleCommunitySelect(community)} className="flex items-center w-full text-left px-4 py-2 hover:bg-gray-100">
                      <MapPin className="h-4 w-4 mr-2 text-news-primary" />
                      <span>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <div className="text-xs text-gray-500 mr-1">Popular:</div>
        {['Tampa, FL', 'Orlando, FL', 'Miami, FL', 'Atlanta, GA', 'New York, NY'].map(location => <button key={location} type="button" onClick={() => setSearchQuery(location)} className="text-xs bg-white hover:bg-gray-100 text-news-primary px-2 py-1 rounded-full border border-gray-200">
            {location}
          </button>)}
      </div>
    </div>;
