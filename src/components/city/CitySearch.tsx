...
export const CitySearch = ({
  ...
  return<div className="bg-white rounded-lg shadow-md border border-gray-200 p-6">
      ...
      <div className="relative">
        <input type="text" placeholder="Enter city name..." value={query} onChange={e =>setQuery(e.target.value)} className="w-full pl-10 pr-10 py-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-news-primary" /><Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-gray-400" />
        {query && <button onClick={() =>setQuery('')} className="absolute right-3 top-1/2 transform -translate-y-1/2"><X className="h-5 w-5 text-gray-400 hover:text-gray-600" />
          </button>}
        {/* Search results */}
        {query.length >= 2 && <div className="absolute left-0 right-0 mt-1 bg-white rounded-md shadow-lg border border-gray-200 z-10 max-h-60 overflow-y-auto">
            ...
            setQuery('');
                      ...
                      <div>
                        ...
                        <div className="text-sm text-gray-500">Pop: {city.population.toLocaleString()} •{' '}
                          {city.activeStories} stories</div>
                      </div>
                      <MapPin className="h-5 w-5 text-gray-400" />
                    </div>
                  </button>)}
              </div> : <div className="p-4 text-center text-gray-500">No communities found matching "{query}"</div>}
          </div>}
      </div>
    </div>;
};