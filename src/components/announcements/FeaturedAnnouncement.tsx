...
export const FeaturedAnnouncement = ({
  ...
  return <div className="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow cursor-pointer">
      ...
      <div className="p-6">
        <div className="flex flex-col md:flex-row">
          {image && <div className="w-full md:w-2/5 mb-4 md:mb-0 md:mr-6">
              <img src={image} alt={title} className="w-full h-64 md:h-56 object-cover rounded-md shadow-sm hover:opacity-90 transition-opacity" />
            </div>}
          <div className={`${image ? 'w-full md:w-3/5' : 'w-full'}`}>
          ...
          }} isLiked={liked} onLike={handleLike} size="md" showReadMore={true} />
          </div>
        </div>
      </div>
    </div>;
...
const getTypeStyles = type => {
  switch (type) {
    ...
    default:
      return {
        headerBg: 'bg-news-primary bg-opacity-10 border-l-4 border-news-primary',
        headerText: 'text-news-primary',
        accentText: 'text-news-primary'
      };
  }
};