      ...
      <div className="flex flex-wrap justify-between border-b border-gray-200 pb-2">
        <div className="flex flex-wrap">
          {allCategories.map(category => <div key={category} onClick={() => onCategorySelect(category)} className={`mr-6 flex cursor-pointer items-center pb-2 text-sm 
                ${activeCategory === category ? 'text-news-primary font-medium border-b-2 border-news-primary' : 'text-gray-600 hover:text-blue-600'}`}>
              <CategoryIcon category={category} />
              <span>{category}</span>
            </div>)}
        </div>
        {toggleAllCategories && <div className="flex items-center">
            <button onClick={toggleAllCategories} className="text-xs text-blue-600 mr-4 hover:underline">
              {showAllCategories ? 'Show less categories' : 'See all categories'}
            </button>
