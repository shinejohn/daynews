...
import { ArrowRight, Award, BookOpen, Building, Calendar, Clock, CloudLightning, MapPin } from 'lucide-react';
export const CollectionThemes = ({
  ...
  return <div className={`rounded-lg shadow-md p-4 ${sepiaMode ? 'bg-amber-100 border border-amber-200' : 'bg-white border border-gray-200'}`}>
      ...
      <div className="space-y-3 mb-4">
        {featuredCollections.map(collection => <button key={collection.id} onClick={() =>onCollectionSelect(collection.id)} className={`w-full flex items-center p-3 rounded-md transition-colors ${activeCollection === collection.id ? collection.color : sepiaMode ? 'bg-amber-50 hover:bg-amber-100' : 'bg-gray-50 hover:bg-gray-100'}`}><div className={`p-2 rounded-full mr-3 ${sepiaMode ? 'bg-amber-200 text-amber-800' : activeCollection === collection.id ? 'bg-white bg-opacity-30' : 'bg-gray-200 text-gray-700'}`}>
              {collection.icon}
            </div>
      ...
      <div className="pt-3 border-t border-gray-200">
        ...
        <div className="space-y-2">
          {anniversaryCollections.map(collection => <button key={collection.id} onClick={() =>onCollectionSelect(collection.id)} className={`w-full flex items-center p-2 rounded-md text-sm ${activeCollection === collection.id ? sepiaMode ? 'bg-amber-200' : 'bg-blue-50' : sepiaMode ? 'hover:bg-amber-50' : 'hover:bg-gray-50'}`}><div className={`p-1 rounded-full mr-2 ${sepiaMode ? 'bg-amber-200 text-amber-800' : 'bg-gray-200 text-gray-700'}`}>
                {collection.icon}
              </div>
              <div className="text-left">
                <div className="font-medium text-gray-900">
