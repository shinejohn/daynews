          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {tones.map(tone => <label key={tone} className="flex items-center space-x-2 cursor-pointer">
                  <input type="radio" name="tone" value={tone} checked={selectedTone === tone} onChange={() =>setSelectedTone(tone)} className="text-news-primary focus:ring-news-primary" /><span className="text-sm text-gray-700">{tone}</span>
                </label>)}
            </div>
          </div>
          <div>
            ...
            <div className="flex space-x-6">
              {styles.map(style => <label key={style} className="flex items-center space-x-2 cursor-pointer">
                  <input type="radio" name="style" value={style} checked={selectedStyle === style} onChange={() =>setSelectedStyle(style)} className="text-news-primary focus:ring-news-primary" /><span className="text-sm text-gray-700">{style}</span>
                </label>)}
            </div>
          </div>
        </div>
